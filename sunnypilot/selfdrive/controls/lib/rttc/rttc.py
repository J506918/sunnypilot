...
class RealTimeTorqueCorrection(LatControlTorqueExtBase):
  ...
  def update_rttc_feedforward(self, CS, params, calibrated_pose) -> None:
    if not self._rttc_enabled:
      return
    ...
    # The "pure" RTTC error response can be too weak for cars whose models were trained
    # with a lack of high-magnitude lateral acceleration data, for which the RTTC model
    ...
    # error as the input to the RTTC model. This is not ideal, and potentially degrades the RTTC
    # accuracy for cars that don't have this issue, but it's necessary until a better RTTC model
